...
export default function StarboxPage() {
  ...
  useEffect(() => {
    ...
    return () => cleanup();
  }, [code, roomId, initGameData, cleanup]);
  ...
  const isMyTurn =
    ...
    !!players[computedTurnIndex]?.isMe &&
    !isCountdownPhase;
  ...
  const canPickAbility =
    initialCountdown <= 0 &&
    (roomInfo?.max_player === 1 ||
      (isMyTurn && !iHavePicked && !pickingAbilityId));
          ...
          {roomInfo?.max_player !== 1 && (
            <div className="mt-2 flex flex-col items-center gap-2">
              ...
              {iHavePicked && !allTurnsDone && (
                <p className="text-sm font-medium text-white/50">Kamu sudah memilih. Menunggu pemain lain...</p>
              )}
            </div>
          )}
