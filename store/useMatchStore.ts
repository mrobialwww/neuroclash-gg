...
export const useMatchStore = create<MatchState>((set, get) => ({
  ...
  syncPlayersFromDB: async (roomId) => {
    ...
    try {
      ...
      const alivePlayers = players.filter((p) => p.is_alive && p.health > 0);
      if (alivePlayers.length <= 1 && players.length > 1) {
        console.log(`[MatchStore] Game ended! Only ${alivePlayers.length} player(s) alive.`);
  ...
  setupRealtimeSubscription: (roomId) => {
    const channel = supabase
      ...
      .on("broadcast", { event: "game_ended" }, (payload) => {
        console.log(`[MatchStore] Game ended broadcast received:`, payload);
        set({ isFinished: true });
      })
      .subscribe();
    ...
    while (!allFinished && attempts < maxAttempts) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Check if game should end (only 1 player alive)
          if (data.game_ended) {
            console.log(`[MatchStore] Game ended! Only 1 player alive or all rounds completed.`);
            set({
              isFinished: true,
              isWaitingForAllBattles: false,
              isAdvancingRound: false,
